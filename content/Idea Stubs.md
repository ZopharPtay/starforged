# Idea Stubs

Seeds and half-formed concepts waiting for a home.

## Locations

### The Condensation Shrine

**Oracle roll:** Descriptor 59 (Mysterious) + Focus 65 (Resource)

A place on a station or settlement where water condenses in ways engineering can't explain.  People collect it with bottles.  A little cult of regulars.

- Why hasn't it been monetized?  Best answer: someone tried, and it stopped.  Moved.  Twice.  Now people are superstitious.  Even the local power structure won't touch it.
- Alternative: the water isn't normal.  Tastes too good.  People who drink it report strange dreams.  Whoever tested it walked away spooked.
- Best fit: a station near a ghost light (white dwarf), or a location with precursor tech underneath.  Ties to Horrors or Precursors truth.
- Not Paxton (wrong vibe; too commercial a station for something unmoneyed to survive without a supernatural reason, and we haven't established a ghost light nearby).

## Characters

### Bad Prosthetic Eye

A character (not Marrow) with a single bad cybernetic eye.  Company-issued replacement that was never calibrated right; gives them headaches.  Either removed it (eye patch / empty socket) or disabled it (dead glass eye that doesn't track).  The kind of detail that defines a character when it's their *only* visible prosthetic rather than one item on a list.

## Vehicles

## Factions

## Miscellaneous

### Celadon Terminus: Return Visit Details

On a future Orris visit, Leight notices compound improvements that weren't there before:
- An awning over the Cascade doorway made from Bulldog hull plating (still stenciled "BULLDOG"; nobody bothered to paint over it).
- The "Cascade by Fontaine" sign has been repainted under the new awning; cleaner this time, maybe a second color.  Someone had paint and ten minutes and a dry surface.
- Other accumulated scrap-built improvements: walkways, scaffolding where terracing crumbled, balconies on the 'racks.

### Clay Pigeons

A fun anecdote someone tells Leight on a future visit: workers occasionally use scrap metal as target practice / entertainment.  Not sanctioned.  Nobody stops it.  "Clay pigeons" equivalent using dead thruster housings or similar, diverted from the pile of junk waiting to be packed into the next spin launcher pod.  Useless scrap goes upwell in the empty space of cargo pods (free recycling; Paxton has smelting facilities, the compound doesn't), so there's always a pile of it waiting to go.

### Marrow as Arms Supplier (Long-term Thread)

If trust develops over multiple visits, Marrow has access to Vanguard military hardware that gets written off to his yard.  He's not doing anything with it now beyond stripping it.  But if a situation arises where someone needs restricted hardware, he knows where it is and has plausible deniability.  Don't foreshadow yet; let the relationship earn it.

### The Fake Starbucks

One of the unkillable franchise cockroaches on R3 hasn't actually been a franchise in years.  Some guy just kept using the equipment and the sign after the corporate operator left.  Nobody from the chain has checked in because nobody at corporate cares about a frontier station in the Outlands.  The menu hasn't updated.  The branding is two redesigns out of date.  He's been selling chickory tea (or whatever) out of branded cups for a decade.

Culture-shock moment for Leight: when he eventually reaches another settlement and tries the "same chain," the drink is completely different.  He's been drinking some dude's personal recipe his whole life thinking it was a brand.
